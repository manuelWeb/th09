...
var myEndFunction = function (){
  ...
  if(this.classList[0] === 'pck1' && this.classList[1] === 'gopk'){
    ...
    init(1,'add')
    // evt PK2
    document.querySelector('.pck2').addEventListener("animationend", myEndFunction);
  }else if(this.classList[0] === 'pck2' && this.classList[1] === 'gopk'){
    init(1,'remove')
    init(2,'add')
    // evt PK3
    document.querySelector('.pck3').addEventListener("animationend", myEndFunction);
  }else if(this.classList[0] === 'pck3' && this.classList[1] === 'gopk'){
    init(2,'remove')
    init(0,'add')
    document.querySelector('.pck1').addEventListener("animationend", myEndFunction);
  }
}
